...
(function () {
  ...
  function registerLinks(root) {
    const candidates = Array.from(root.querySelectorAll("a[data-lightbox]"));
    const ordered = [
      ...candidates.filter((link) => !link.closest(".gallery-hero")),
      ...candidates.filter((link) => link.closest(".gallery-hero")),
    ];
    ordered.forEach((link) => {
      if (link.dataset.lightboxBound) return;
      link.dataset.lightboxBound = "true";
      const key = link.dataset.lightboxKey;
      let i = key ? indexByItem.get(key) : undefined;
      if (i === undefined) {
        i = links.length;
        links.push(link);
        if (key) indexByItem.set(key, i);
      }
      if (link.dataset.id && !indexByID.has(link.dataset.id)) indexByID.set(link.dataset.id, i);
  ...
  function render(i) {
    ...
    if (tags) {
      ...
      const source = link.closest("figure, .gallery-hero")?.querySelector(".lightbox-tags-source");
